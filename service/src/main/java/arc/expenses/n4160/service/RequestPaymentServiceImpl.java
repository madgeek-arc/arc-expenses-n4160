...
public class RequestPaymentServiceImpl extends GenericService<RequestPayment> {
    ...
    private StateMachine<NormalStages, StageEvents> build(RequestPayment payment){

        StateMachine<NormalStages, StageEvents> sm = this.factory.getStateMachine(payment.getId());
        sm.stop();

        sm.getStateMachineAccessor()
                .doWithAllRegions(sma -> {

                    sma.addStateMachineInterceptor( new StateMachineInterceptorAdapter<NormalStages, StageEvents>(){
                    ...
                    sma.resetStateMachine(new DefaultStateMachineContext<>(
                            NormalStages.valueOf((payment.getCurrentStage() == null ? NormalStages.Stage7.name() : payment.getCurrentStage())), null, null, null));
    ...
    public void approve(RequestPayment requestPayment, HttpServletRequest req) {
        ...
        StateMachine<NormalStages, StageEvents> sm = this.build(requestPayment);
    ...
    public RequestPayment createPayment(Request request) throws Exception {
        ...
        requestPayment.setCurrentStage(NormalStages.Stage7.name());
